...
pygame.display.set_caption("Space Invader - made by Ngoc Tien")
...
class Bullet:
    ...
    def show(self):
        surface.blit(pygame.transform.rotate(
            bulletImg, self.angle), (self.x + 16, self.y))
...
class Rocket(Bullet):
    ...
    def show(self):
        ...
        surface.blit(pygame.transform.rotate(rocketImg, 90),
                     (2 * self.oldX - self.x + 64, self.y))
...
def showScore(score):
    value = score_font.render(
        "Your Score: " + str(score), True, (255, 255, 102))
...
if __name__ == '__main__':
    ...
    for _ in range(numberOfEnemy):
        enemyList.append(Enemy(random.randint(0, 736),
                         random.randint(0, 150), 0.4, 0))
